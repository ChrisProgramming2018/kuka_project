...
def train_agent(args):
    ...
    pathname += "_lr_critc_" + str(args.lr_critic) + "_discount_" + str(args.discount)
    arg_text = str(args)
    ...
    tb_update_counter = 0
    # load replaybuffer
    model_path = "pretrained/kuka_block_grasping-v0-13913reward_-1.11-agentTCQ"
    ...
    while total_timesteps <  args.max_timesteps:
        ...
        if total_timesteps > args.start_timesteps:
            for i in range(args.repeat_update):
                policy.train(replay_buffer, rb_expert, writer, 1)
        episode_timesteps += 1
        total_timesteps += 1
