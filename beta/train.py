...
def create_next_obs(state, size, args, state_buffer, debug=False):
...
def stacked_frames(state, size, args, debug=False):
...
def train_agent(args):
    ...
    tensorboard_name += "_2"
    writer1 = SummaryWriter(tensorboard_name)
    tensorboard_name += "_3"
    writer2 = SummaryWriter(tensorboard_name)
    size = args.size
    ...
    policy1 = TQC(state_dim, action_dim, max_action, 0, args)    
    policy2 = TQC(state_dim, action_dim, max_action, 3, args)    
    policy3 = TQC(state_dim, action_dim, max_action, 5, args)
    agents = [policy1, policy2, policy3]
    ...
    tb_update_counter = 0
    # TODO: evaluate 
    evaluations.append(evaluate_policy(policy1, writer, total_timesteps, args, env))
    ...
    while total_timesteps <  args.max_timesteps:
        ...
        if done:
            ...
            if total_timesteps > args.start_timesteps and episode_num % args.update_beta_freq == 0:
                policy1.update_beta(replay_buffer, writer, total_timesteps)
                policy2.update_beta(replay_buffer, writer, total_timesteps)
                policy3.update_beta(replay_buffer, writer, total_timesteps)
                replay_buffer.k = 0
                current_agent = np.argmin([policy1.delta, policy2.delta, policy3.delta]) 
                text = "dif1: {} dif2: {} dif3: {}  new agent: {} ".format(policy1.delta, policy2.delta, policy3.delta, current_agent) 
                print(text)
            ...
            if timesteps_since_eval >= args.eval_freq:
                ...
                evaluations.append(evaluate_policy(agents[current_agent], writer, total_timesteps, args,  env))
                torch.manual_seed(args.seed)
                np.random.seed(args.seed)
                save_model = file_name + '-{}reward_{:.2f}-agent{}'.format(episode_num, evaluations[-1], args.policy) 
                agents[current_agent].save(save_model)
            # When the training step is done, we reset the state of the environment
            state = env.reset()
            obs, state_buffer = stacked_frames(state, size, args)
        ...
        if total_timesteps < args.start_timesteps:
            ...
            action = agents[current_agent].select_action(obs)
        ...
        new_obs, state_buffer = create_next_obs(new_obs, size, args, state_buffer)
        ...
        if total_timesteps > args.start_timesteps:
            for i in range(args.repeat_update):
                agents[current_agent].train(replay_buffer, writer, 1)
                next_agent = (current_agent + 1) % 3
                prev_agent = (current_agent - 1) % 3
                agents[next_agent].transfer_weights(agents[current_agent])
                agents[prev_agent].transfer_weights(agents[current_agent])
        episode_timesteps += 1
        total_timesteps += 1
